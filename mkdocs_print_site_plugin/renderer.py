...
class Renderer(object):
    ...
    def get_toc_sidebar(self) -> TableOfContents:
        ...
        (toc, _) = self._toc_sidebar_layer(
            self._get_items(), excluded_pages, section_numbers=[], chapter_number=0
        )
    ...
    def _toc_sidebar_layer(
        ...
        Returns the items generated, and the last chapter number used.
        """
        ...
        for item in items:
            ...
            if item.is_section:
                ...
                if self.plugin_config.get("enumerate_headings"):
                    section = ".".join(
                        [int_to_roman(n) for n in section_numbers + [current_section]]
                    )
                    title = f"{section}. {item.title}"
                else:
                    title = item.title
                ...
                (child_toc, chapter_number) = self._toc_sidebar_layer(
                    item.children,
                    excluded_pages,
                    section_numbers + [current_section],
                    chapter_number,
                )
                section_link.children.extend(child_toc)

        return (toc, chapter_number)
